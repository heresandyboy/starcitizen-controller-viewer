...
interface ConfigUploaderProps {
  onConfigLoaded: (config: ConfigState) => void;
}
...
export function ConfigUploader({ onConfigLoaded }: ConfigUploaderProps) {
  ...
  const handleFiles = useCallback(async (newFiles: FileList | File[]) => {
    ...
    // Auto-parse when both files are present
    if (updatedFiles.rewasd && updatedFiles.xml) {
      await parseConfigs(updatedFiles);
    }
  }, [files]);

  const parseConfigs = async (filesToParse: FileState) => {
    if (!filesToParse.rewasd || !filesToParse.xml) return;
    ...
    try {
      ...
      const result = parseAndResolve(rewasdText, xmlText);

      if (result.errors.length > 0) {
        setError(result.errors.join('\n'));
        // Continue anyway - errors might be non-fatal warnings
      }
      ...
      onConfigLoaded({
        ...
        availableModes: getAvailableModes(result.mappings),
        availableModifiers: getAvailableModifiers(result.mappings),
      });
    } catch (err) {
  ...
  return (
    <div className="w-full">
      ...
      {/* Manual parse button if both files present but not auto-parsed */}
      {files.rewasd && files.xml && !isLoading && (
        <button
          onClick={() => parseConfigs(files)}
...
function FileStatus({ label, file, required }: FileStatusProps) {
  return (
